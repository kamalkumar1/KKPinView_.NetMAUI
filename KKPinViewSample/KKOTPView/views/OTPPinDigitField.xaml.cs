...
public sealed partial class OTPPinDigitField : ContentView
{
    public static readonly BindableProperty DigitProperty = BindableProperty.Create(
        nameof(Digit), typeof(string), typeof(OTPPinDigitField), string.Empty);

    public static readonly BindableProperty IsFilledProperty = BindableProperty.Create(
        nameof(IsFilled), typeof(bool), typeof(OTPPinDigitField), false, propertyChanged: OnIsFilledChanged);

    public static readonly BindableProperty FieldWidthProperty = BindableProperty.Create(
        nameof(FieldWidth), typeof(double), typeof(OTPPinDigitField), KKPinviewConstant.FieldWidth);

    public static readonly BindableProperty FieldHeightProperty = BindableProperty.Create(
        nameof(FieldHeight), typeof(double), typeof(OTPPinDigitField), KKPinviewConstant.FieldHeight);

    public static readonly BindableProperty FontSizeProperty = BindableProperty.Create(
        nameof(FontSize), typeof(double), typeof(OTPPinDigitField), KKPinviewConstant.DigitFontSize);

    public static readonly BindableProperty DigitFontAttributesProperty = BindableProperty.Create(
        nameof(DigitFontAttributes), typeof(FontAttributes), typeof(OTPPinDigitField), KKPinviewConstant.DigitFontAttributes);

    public static readonly BindableProperty TextColorProperty = BindableProperty.Create(
        nameof(TextColor), typeof(MauiColor), typeof(OTPPinDigitField), KKPinviewConstant.TextColor);

    public static new readonly BindableProperty BackgroundColorProperty = BindableProperty.Create(
        nameof(BackgroundColor), typeof(MauiColor), typeof(OTPPinDigitField), KKPinviewConstant.DigitFieldBackgroundColor);

    public static readonly BindableProperty BorderColorProperty = BindableProperty.Create(
        nameof(BorderColor), typeof(MauiColor), typeof(OTPPinDigitField), KKPinviewConstant.DigitFieldEmptyBorderColor, propertyChanged: OnBorderColorChanged);

    public static readonly BindableProperty CornerRadiusProperty = BindableProperty.Create(
        nameof(CornerRadius), typeof(double), typeof(OTPPinDigitField), KKPinviewConstant.FieldCornerRadius, propertyChanged: OnCornerRadiusChanged);

    public static readonly BindableProperty FieldShapeTypeProperty = BindableProperty.Create(
        nameof(FieldShapeType), typeof(KKPinFieldShapeType), typeof(OTPPinDigitField), KKPinviewConstant.FieldShapeType, propertyChanged: OnShapeChanged);

    public static readonly BindableProperty IsSecureProperty = BindableProperty.Create(
        nameof(IsSecure), typeof(bool), typeof(OTPPinDigitField), KKPinviewConstant.PinFieldIsSecure);

    public static readonly BindableProperty MaxLengthProperty = BindableProperty.Create(
        nameof(MaxLength), typeof(int), typeof(OTPPinDigitField), 1);

    /// <summary>Max length for the entry. Use 1 for single-digit, or OTP length for paste support.</summary>
    ...
    /// <summary>Raised when user pastes multiple characters (full OTP). Parent should distribute across fields.</summary>
    public event EventHandler<string>? PasteReceived;
    ...
    {
        if (bindable is OTPPinDigitField field)
        {
    ...
    {
        if (bindable is OTPPinDigitField field && newValue is MauiColor color)
        {
    ...
    private const string BorderAnimationName = "OTPPinDigitFieldBorder";
    ...
    private void OnDigitEntryTextChanged(object? sender, TextChangedEventArgs e)
    {
        ...
        // Paste: multiple characters - extract digits and raise PasteReceived for parent to handle
        if (newText.Length > 1)
        {
            ...
            if (!string.IsNullOrEmpty(digitsOnly))
            {
                ...
                Dispatcher.DispatchDelayed(TimeSpan.Zero, () => PasteReceived?.Invoke(this, digitsOnly));
            }
            return;
        }
    ...
    /// <summary>
    /// Enables iOS one-time-code autocomplete for SMS OTP. Call on the first field only.
    /// </summary>
    internal void EnableOneTimeCodeContentType()
    {
#if IOS
        void Apply()
        {
            if (DigitEntry?.Handler?.PlatformView is UIKit.UITextField tf)
                tf.TextContentType = UIKit.UITextContentType.OneTimeCode;
        }
        HandlerChanged += (_, _) => Apply();
        Apply();
#endif
    }
}
